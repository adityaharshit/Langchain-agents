        ...
        try:
            ...
            response = await client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert research analyst specializing in breaking down complex research questions into systematic investigation plans.",
                    },
                    ...
                    subqueries.append(
                        {
                            ...
                            "expected_info_type": sq_data.get(
                                "expected_info_type", "analysis"
                            ),
        ...
        try:
            ...
            response = await client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert research strategist specializing in optimizing research workflows and task prioritization for maximum efficiency and impact.",
                    },
                    {"role": "user", "content": prompt},
                ],
                temperature=0.2,
                    ...
                    prioritized_tasks.append(
                        {
                            ...
                            "estimated_effort": task_data.get(
                                "estimated_effort", "medium"
                            ),
...
async def progress_tracker_tool(tool_input: dict, context: dict) -> dict:
    ...
    try:
        ...
        return {
            "status": "ok",
            "result": {"event_emitted": True, "event": progress_event},
            "meta": {"queue_size": progress_queue.qsize()},
        }
...
async def result_synthesis_tool(tool_input: dict, context: dict) -> dict:
    ...
    try:
        ...
        if not agent_results:
            return {
                "status": "error",
                "error": "Agent results are required",
                "meta": {},
            }
...
async def _simple_synthesis(results_by_agent: Dict[str, List[dict]]) -> dict:
    ...
    synthesis = {"combined_results": [], "agent_contributions": {}, "total_results": 0}
        ...
        try:
            ...
            response = await client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert research assistant specializing in finding authoritative sources for academic and policy research.",
                    },
                    {"role": "user", "content": prompt},
                ],
            ...
            if len(generated_urls) < max_urls // 2:
                fallback_urls = await _generate_fallback_urls(
                    query, subqueries, max_urls - len(generated_urls)
                )
...
async def _generate_fallback_urls(
    query: str, subqueries: List[str], max_urls: int
) -> List[dict]:
    """
                    ...
                    documents.append(
                        {
                            ...
                            "snippet": row.cleaned_text[:200] + "..."
                            if len(row.cleaned_text) > 200
...
async def _load_url_with_timeout(
    url: str, timeout: int, retry_count: int = 0, max_retries: int = 2
) -> Optional[dict]:
    """
    ...
    try:
        logger.info(
            f"Attempting to scrape: {url} (attempt {retry_count + 1}/{max_retries + 1})"
        )
        ...
        documents = await asyncio.wait_for(
            asyncio.to_thread(loader.load), timeout=timeout
        )
        ...
        if documents and len(documents) > 0:
            ...
            if content:
                logger.info(
                    f"Successfully scraped: {url} ({len(content)} chars in {scraping_time:.2f}s)"
                )
                return {
                    "url": url_result,
                    "title": title,
        ...
        logger.warning(
            f"URL {url} timed out after {timeout}s (attempt {retry_count + 1}/{max_retries + 1})"
        )
        ...
        if retry_count < max_retries:
            ...
            return await _load_url_with_timeout(
                url, timeout, retry_count + 1, max_retries
            )
        else:
            logger.error(
                f"Failed to scrape {url} after {max_retries + 1} attempts: Timeout"
            )
            return None
        ...
        logger.warning(
            f"Error scraping {url} (attempt {retry_count + 1}/{max_retries + 1}): {error_type} - {str(e)}"
        )

        # Retry for transient errors
        if (
            retry_count < max_retries
            and "ConnectionError" in error_type
            or "HTTPError" in error_type
        ):
            delay = config.BACKOFF_FACTOR**retry_count
            logger.warning(f"Retry {retry_count + 1} for {url} after {delay}s delay")
            await asyncio.sleep(delay)
            ...
            logger.error(
                f"Failed to scrape {url} after {retry_count + 1} attempts: {error_type} - {str(e)}"
            )
            return None
...
async def web_scraper_tool(tool_input: dict, context: dict) -> dict:
    ...
    try:
        ...
        logger.info(
            f"Scraping complete: {len(scraped_documents)}/{len(urls)} successful, "
...
@mcp_tool(
    ...
    allowed_agents=["WebScraperRetrievalAgent", "DeepAnalysisAgent", "ResearchCoordinatorAgent"],
)
async def semantic_search_tool(tool_input: dict, context: dict) -> dict:
    ...
    try:
                ...
                relevant_results.append(
                    {
                        ...
                        "title": doc.metadata.get("title", ""),
                        "source_trust_score": doc.metadata.get(
        ...
        if relevant_results:
            avg_similarity = sum(r["similarity_score"] for r in relevant_results) / len(
                relevant_results
            )
...
async def rag_upsert_tool(tool_input: dict, context: dict) -> dict:
    ...
    try:
            ...
            if content:
                langchain_doc = LangChainDocument(
                    ...
                    metadata={
                        ...
                        "extraction_method": doc_data.get(
            ...
            processed_documents.append(
                {
                    ...
                    "chunks_created": len(
                        [
                            c
                            for c in chunks
                            if c.metadata.get("url") == doc.metadata.get("url")
                        ]
                    ),
                }
            )
        ...
        return {"status": "error", "error": f"RAG upsert failed: {str(e)}", "meta": {}}
...
async def comprehensive_analysis_tool(tool_input: dict, context: dict) -> dict:
    ...
    try:
        ...
        if not context_documents:
            return {
                "status": "error",
                "error": "Context documents are required",
                "meta": {},
            }
        ...
        try:
            ...
            response = await client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert research analyst specializing in comprehensive policy and academic analysis. You create detailed, well-structured reports with proper academic rigor.",
                    },
                    {"role": "user", "content": prompt},
                ],
                temperature=0.3,
    ...
    try:
        ...
        for metric in metrics:
            ...
            if len(metric_values) >= 2:
                ...
                trend_analysis["change_rate"] = (
                    (end_value - start_value) / start_value if start_value != 0 else 0
                )

                # Calculate volatility (standard deviation)
                mean_value = sum(metric_values) / len(metric_values)
                variance = sum((x - mean_value) ** 2 for x in metric_values) / len(
                    metric_values
                )
                ...
                for i in range(1, len(metric_values)):
                    change = (
                        abs(metric_values[i] - metric_values[i - 1])
                        / metric_values[i - 1]
...
async def causal_reasoning_tool(tool_input: dict, context: dict) -> dict:
    ...
    try:
        ...
        if not events:
            return {
                "status": "error",
                "error": "Events are required for causal analysis",
                "meta": {},
            }
...
async def statistical_analysis_tool(tool_input: dict, context: dict) -> dict:
    ...
    try:
        ...
        for dataset_name, data in (
            datasets.items() if isinstance(datasets, dict) else enumerate(datasets)
        ):
...
async def _analyze_causal_relationship(
    events: List[dict], cause: str, effect: str
) -> dict:
    ...
    if cause_events and effect_events:
        ...
        strength = min(len(cause_events), len(effect_events)) / max(
            len(cause_events), len(effect_events)
        )
        ...
        for j, (name2, data2) in enumerate(dataset_items[i + 1 :], i + 1):
            if (
                ...
                if min_len > 1:
                    ...
                    numerator = sum(
                        (x - mean1) * (y - mean2)
                        for x, y in zip(corr_data1, corr_data2)
                    )
                    denominator = (
                        sum((x - mean1) ** 2 for x in corr_data1)
                        * sum((y - mean2) ** 2 for y in corr_data2)
                    ) ** 0.5
        ...
        try:
            ...
            response = await client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert information literacy specialist with extensive experience in evaluating source credibility for academic and professional research.",
                    },
                    {"role": "user", "content": prompt},
                ],
                temperature=0.2,
                    ...
                    credibility_scores.append(
                        {
                            ...
                            "credibility_score": cred_data.get(
            ...
            avg_credibility = (
                sum(s["credibility_score"] for s in credibility_scores)
                / len(credibility_scores)
                if credibility_scores
                else 0
            )
            high_credibility_sources = [
                s for s in credibility_scores if s["credibility_score"] > 0.7
            ]
...
async def cross_reference_validator_tool(tool_input: dict, context: dict) -> dict:
    ...
    try:
        ...
        if not claims:
            return {
                "status": "error",
                "error": "Claims are required for validation",
                "meta": {},
            }
        ...
        for claim in claims:
            ...
            for source in sources:
                ...
                support_score = await _calculate_claim_support(
                    claim_text, source_content
                )

                if support_score > 0.7:
                    supporting_sources.append(
                        {"source": source, "support_score": support_score}
                    )
                elif support_score < 0.3:
                    contradicting_sources.append(
                        {"source": source, "contradiction_score": 1.0 - support_score}
                    )
                else:
                    neutral_sources.append(
                        {"source": source, "relevance_score": support_score}
                    )
            ...
            support_ratio = (
                len(supporting_sources) / total_sources if total_sources > 0 else 0
            )

            validation_confidence = support_ratio
            validation_status = (
                "validated"
                if validation_confidence >= validation_threshold
                else "unvalidated"
            )
        ...
        return {
            ...
            "result": {
                ...
                "contradicted_claims": [
                    r
                    for r in validation_results
                    if r["validation_status"] == "contradicted"
                ],
            },
            "meta": {
...
async def contradiction_detector_tool(tool_input: dict, context: dict) -> dict:
    ...
    try:
        ...
        if len(statements) < 2:
            return {
                "status": "error",
                "error": "At least 2 statements required for contradiction detection",
                "meta": {},
            }
                    ...
                    contradictions.append(
                        {
                            ...
                            "contradiction_type": await _classify_contradiction_type(
                                stmt1.get("text", ""), stmt2.get("text", "")
                            ),
                            "sources": [
                                stmt1.get("source", ""),
                                stmt2.get("source", ""),
                            ...
                            * 0.9,  # Slightly reduce confidence
                        }
                    )
        ...
        return {
            ...
            "result": {
                ...
                "contradiction_rate": len(contradictions)
                / (len(statements) * (len(statements) - 1) / 2),
            },
            "meta": {
...
async def confidence_scorer_tool(tool_input: dict, context: dict) -> dict:
    ...
    try:
        ...
        scoring_factors = tool_input.get(
            "scoring_factors", ["source_quality", "evidence_strength", "consistency"]
        )

        if not results:
            return {
                "status": "error",
                "error": "Results are required for confidence scoring",
                "meta": {},
            }
        ...
        for result in results:
            ...
            if "source_quality" in scoring_factors:
                source_quality = await _evaluate_source_quality(
                    result.get("sources", [])
                )
            ...
            if "evidence_strength" in scoring_factors:
                evidence_strength = await _evaluate_evidence_strength(
                    result.get("evidence", [])
                )
            ...
            final_confidence = (
                total_confidence / max_confidence if max_confidence > 0 else 0.0
            )
            ...
            confidence_scores.append(
                {
                    ...
                    "reliability_assessment": await _generate_reliability_assessment(
                        final_confidence, factor_scores
                    ),
                }
            )
        ...
        return {
            ...
            "result": {
                ...
                "high_confidence_results": [
                    s for s in confidence_scores if s["confidence_score"] > 0.8
                ],
            },
            "meta": {"scoring_factors": scoring_factors, "total_results": len(results)},
        }
...
async def _generate_credibility_recommendations(
    score: float, components: dict
) -> List[str]:
    ...
    if score < 0.5:
        recommendations.append(
            "Consider finding additional sources to verify information"
        )

    if components.get("domain_authority", 0) < 0.5:
        recommendations.append(
            "Source domain has low authority - verify with established sources"
        )

    if components.get("publication_recency", 0) < 0.5:
        recommendations.append(
            "Information may be outdated - check for more recent sources"
        )
...
async def _detect_contradiction(
    stmt1: str, stmt2: str, method: str = "semantic"
) -> float:
...
async def _classify_contradiction_type(stmt1: str, stmt2: str) -> str:
    ...
    if "increase" in stmt1.lower() and "decrease" in stmt2.lower():
        return "directional"
    elif any(word in stmt1.lower() for word in ["yes", "true", "correct"]) and any(
        word in stmt2.lower() for word in ["no", "false", "incorrect"]
    ):
        return "boolean"
    else:
        return "semantic"
...
async def _evaluate_evidence_strength(evidence: List[dict]) -> float:
    ...
    )  # More evidence = stronger (up to 5 pieces)
...
async def _evaluate_consistency(claims: List[dict]) -> float:
    ...
    for i in range(len(claims)):
        for j in range(i + 1, len(claims)):
            total_pairs += 1
            contradiction_score = await _detect_contradiction(
                claims[i].get("text", ""), claims[j].get("text", "")
            )
...
async def citation_formatter_tool(tool_input: dict, context: dict) -> dict:
    ...
    try:
        ...
        if not sources:
            return {
                "status": "error",
                "error": "Sources are required for citation formatting",
                "meta": {},
            }
        ...
        return {
            ...
            "meta": {"citation_style": citation_style, "formatting_standard": "IEEE"},
        }
...
async def report_structuring_tool(tool_input: dict, context: dict) -> dict:
    ...
    try:
        ...
        if not content:
            return {
                "status": "error",
                "error": "Content is required for report structuring",
                "meta": {},
            }
        ...
        return {
            ...
            "result": {
                ...
                "formatting_guidelines": await _get_formatting_guidelines(
                    structure_type
                ),
            },
            "meta": {
...
async def executive_summary_generator_tool(tool_input: dict, context: dict) -> dict:
    ...
    try:
        ...
        summary_components = await _extract_summary_components(
        ...
        executive_summary = {
            ...
            "confidence_assessment": await _generate_confidence_assessment(
                analysis_results
            ),
            "metadata": {
...
async def _extract_summary_components(
    analysis_results: dict, key_points: List[str]
) -> dict:
    ...
    if "evidence" in analysis_results:
        components["evidence"] = analysis_results["evidence"][
            :3
...
async def _generate_overview(components: dict, params: dict) -> str:
    ...
    confidence = components.get("confidence_indicators", {}).get(
    ...
    if confidence > 0.7:
        ...
        overview_parts.append(
            "This analysis provides important findings with moderate confidence."
        )
    else:
        overview_parts.append(
            "This preliminary analysis offers initial insights that require further validation."
        )
    ...
    if findings:
        ...
        overview_parts.append(
            f"The primary finding indicates that {top_finding.lower()}"
        )
...
async def _generate_recommendations(components: dict) -> List[str]:
    ...
    if confidence < 0.5:
        recommendations.append(
            "Conduct additional research to validate preliminary findings"
        )

    if len(findings) > 3:
        recommendations.append(
            "Focus on the most significant findings for immediate action"
        )
...
async def _format_executive_summary(summary: dict) -> str:
    ...
    formatted_parts.append(
        f"Overall Confidence: {confidence['confidence_level'].title()} ({confidence['overall_confidence']:.2f})"
    )
